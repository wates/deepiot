...
int fc0012_set_freq(void *dev, uint32_t freq) {
    ...
    rtlsdr_set_gpio_bit((rtlsdr_dev_t*)dev, 6, (freq > 300000000) ? 1 : 0);
...
int fc2580_init(void *dev) { return 0;/*fc2580_Initialize(dev);*/ }
int fc2580_exit(void *dev) { return 0; }
int _fc2580_set_freq(void *dev, uint32_t freq) {
	return 0;/*fc2580_SetRfFreqHz(dev, freq);*/
}
int fc2580_set_bw(void *dev, int bw) { return 0;/*fc2580_SetBandwidthMode(dev, 1);*/ }
...
int rtlsdr_read_array(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t *array, uint8_t len)
{
    ...
    r=dev->devh->Control(CTRL_IN,0,addr,index,array,len);
...
int rtlsdr_write_array(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t *array, uint8_t len)
{
    ...
    r=dev->devh->Control(CTRL_OUT,0,addr,index,array,len);
...
uint16_t rtlsdr_read_reg(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t len)
{
    ...
    r=dev->devh->Control(CTRL_IN, 0, addr, index, data, len);
...
int rtlsdr_write_reg(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint16_t val, uint8_t len)
{
    ...
    r=dev->devh->Control(CTRL_OUT, 0, addr, index, data, len);
...
int rtlsdr_open(rtlsdr_dev_t **out_dev, uint32_t index)
{
	...
	dev = reinterpret_cast<rtlsdr_dev_t*>(malloc(sizeof(rtlsdr_dev_t)));
	if (NULL == dev)
		return -ENOMEM;
  ...
  GUID gp = { 0x285a8776, 0x6547, 0x414e, { 0xa8, 0x29, 0x6f, 0xe, 0xa5, 0xa3, 0x3e, 0x67 } };
  dev->devh = NakedUsb::Create();
  if (dev->devh){
    dev->devh->Open(&gp);
  }
...
int rtlsdr_read_sync(rtlsdr_dev_t *dev, void *buf, int len, int *n_read)
{
	if (!dev)
		return -1;
  for (auto &ep : dev->devh->Endpoints())
  {
    if (ep.input && NakedUsb::EndpointType::Bulk == ep.type){
      *n_read = dev->devh->ReadSync(&ep,static_cast<uint8_t*>(buf), len);
      return *n_read;
    }
  }
  return -1;
...
static int _rtlsdr_alloc_async_buffers(rtlsdr_dev_t *dev)
{
	...
	if (!dev->xfer) {
		dev->xfer = static_cast<libusb_transfer**>(malloc(dev->xfer_buf_num *
	...
	if (!dev->xfer_buf) {
		dev->xfer_buf = static_cast<unsigned char**>(malloc(dev->xfer_buf_num *
					   sizeof(unsigned char *)));

		for(i = 0; i < dev->xfer_buf_num; ++i)
			dev->xfer_buf[i] = static_cast<unsigned char*>(malloc(dev->xfer_buf_len));
	}
